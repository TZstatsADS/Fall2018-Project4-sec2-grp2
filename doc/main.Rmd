...
for (i in 1:length(error_set)){
  ...
  writeLines(substitution[[i]], sub('/error/', '/substituted/', error_dir[i]))
}
...
for (i in 1:length(new_doc)){
  new_doc[[i]] <- readLines(new_doc_names, encoding = 'UTF-8')
  new_doc[[i]][match(ifcleanind[[i]] == TRUE)] <- substitution[[i]]
  writeLines(new_doc[[i]], paste('../output/new_doc/', error_names[i], sep = ''), encoding = 'UTF-8')
}
```
...
correct_path <- "../output/substituted/"
